 ...
 * 	or full match 
 */
module.exports = function textSearchFilter(searchString, properties, fullMatchOnly = false) {
	let finalObject = {};
	...
	if (wordsToFind.length > 1) {
		do {
			...
			if (regexOperator.test(extractedWord)) {
				...
				extractedWord = replaceChar(extractedWord, 1, "ª", " ");
				//identifies if finalObject is empty to evade possible {} object
				if (isEmpty(finalObject)) {
					finalObject = buildOrObject(properties, escapeChar(extractedWord), fullMatchOnly);
				} else {
					//If finalObject is not empty, it makes it grow.
					let noArgObject = buildOrObject(properties, escapeChar(extractedWord), fullMatchOnly);
					let filterPlaceHolder = finalObject;
					finalObject = {
		...
		let word = replaceChar(wordsToFind[0], 1, "ª", " ");
