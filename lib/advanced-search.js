...
const { getArg, getNotAtStart, replaceDoubleQuotes } = require('./build-object-functions');
...
module.exports = function advancedSearchFilter(searchString) {
	...
	if (operatorArray.length === 0) {
		filterObject = getArg(argArray[0], filterObject);
	} else if (/^NOT$/i.test(operatorArray[0]) && operatorArray.length === 1 && argArray.length === 1) {
		filterObject = getNotAtStart(argArray[0], filterObject);
	} else {
